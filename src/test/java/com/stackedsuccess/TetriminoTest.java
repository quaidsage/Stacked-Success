...
class TetriminoTest {
  ...
  void testCollisionWithBoardBottomBoundary() {
    ...
    assertFalse(
        gameBoard.checkCollision(0, 1),
        "Tetrimino should not collide when moving down while it is in bounds");
    t.setYPos(gameBoard.getHeight() - t.getHeight());
    assertTrue(
        gameBoard.checkCollision(0, gameBoard.getHeight()),
        "Tetrimino should collide when moving down out of bounds");
  }
}
